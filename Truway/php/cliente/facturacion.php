...
if ($id_usuario) {
    ...
    while ($col = mysqli_fetch_assoc($res_carrito)) {
        ...
        if ($col['fecha_reserva']) {
            if (!isset($fechas_reserva_por_producto[$id_producto])) {
                $fechas_reserva_por_producto[$id_producto] = [];
            }
            $fechas_reserva_por_producto[$id_producto][] = $col['fecha_reserva'];
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id_usuario && count($productos_cantidades) > 0 && $nombre_titular && $numero_tarjeta && $vencimiento && $cvv) {
        ...
        if (mysqli_query($conexion, $sql)) {
            ...
            $hoyT = strtotime($fecha);
            foreach ($productos_cantidades as $id_prod => $cantidad) {
                $id_prod = intval($id_prod);
                $cantidad = intval($cantidad);

                // Buscar la fecha de reserva más cercana a hoy para este producto
                $fecha_detalle = $fecha;
                if (isset($fechas_reserva_por_producto[$id_prod]) && count($fechas_reserva_por_producto[$id_prod]) > 0) {
                    $min_dif = null;
                    foreach ($fechas_reserva_por_producto[$id_prod] as $f) {
                        $fechaT = strtotime($f);
                        $dif = abs($fechaT - $hoyT);
                        if ($min_dif === null || $dif < $min_dif) {
                            $min_dif = $dif;
                            $fecha_detalle = date('Y-m-d', $fechaT);
                        }
                    }
                }

                mysqli_query($conexion, "INSERT INTO detalle_pedido (id_pedido, id_producto, cantidad, fecha) VALUES ($id_pedido, $id_prod, $cantidad, '$fecha_detalle')");
            }
...
<main>
    ...
    <section class="facturacion">
        ...
        <form method="post" class="form-facturacion">
             <div class="cont-input">
                ...
                <div class="seleccion-tarjeta">
                    <label class="cont-tipo-tarjeta">
                        ...
                        <input checked type="radio" name="tipo_tarjeta" value="debito"> Débito
                    </label>
                    <label class="cont-tipo-tarjeta">
                        ...
                        <input type="radio" name="tipo_tarjeta" value="credito"> Crédito   
                    </label>
                </div>
            </div>
            ...
            <div class="cont-input">
                <label for="numero_tarjeta" class="lbl">Número de tarjeta</label>
